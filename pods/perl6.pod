...
They represent a single value. They start with a `$`
...
They represent multiple values. Their name start with `@`.
...
=head2 * Subs (subroutines, or functions in most other languages).
...
=item You can call a function with an array using the "argument list flattening" operator `|` (it's not actually the only role of this operator, but it's one of them)
...
=item It can also have optional arguments:
...
=item You can also give them a default value when they're not passed:
...
=item You can also, by using a syntax akin to the one of hashes (yay unification !),  pass *named* arguments to a `sub`.
...
There's one gotcha to be aware of, here: If you quote your key, Perl 6 won't be able to see it at compile time, and you'll have a single Pair object as a positional paramater.
...
=item If a sub takes a named boolean argument ...
...
=item You can also provide your named arguments with defaults:
...
- `if` Before talking about `if`, we need to know which values are "Truthy" (represent True), and which are "Falsey" (or "Falsy") -- represent False.  Only these values are Falsey: (), 0, "0", Nil, A type (like `Str` or `Int`), and of course False itself.  Every other value is Truthy.
...
Since Perl languages are very much operator-based languages Perl 6 operators are actually just funny-looking subroutines, in syntactic categories, like infix:<+> (addition) or prefix:<!> (bool not).

=head2 The categories are :
...
=head2 The associativity and precedence list are explained below.

Alright, you're set to go !

=head2 * Equality Checking
...
=head2 范围的构造 ( Range constructors )
...
=head2 * Short-circuit (and tight) versions of the above
...
=head2 The last expression of a sub is returned automatically
...
=head2 You can create a lambda with `-> {}` ("pointy block") or `{}` ("block")
...
=head2 Perl 6 有一个全面系统的对象模型 

定义类使用 `class` 关键字, 成员属性通过 `has` 声明, 方法是通过 `method`. 每个属性都可以是 private 的, 通过使用 `$!attr` 这种方式, 但你可以直接使用 `$.` 来取得  public ( 不可变的 ) 的存取器.
You declare a class with the keyword `class`, fields with `has`, methods with `method`. Every field to private, and is named `$!attr`, but you have `$.` to get a public (immutable) accessor along with it.  (using `$.` is like using `$!` plus a `method` with the same name) 

(Perl 6's object model ("SixModel") is very flexible, and allows you to dynamically add methods, change semantics, etc -- This will not be covered here, and you should refer to the Synopsis)
...
=head2 Perl 6 also has inheritance (along with multiple inheritance)
...
=head2 Roles are supported too (also called Mixins in other languages)
...
=head2 * `our` (happens at `INIT` time -- see "Phasers" below)
...
=head2 * `constant` (happens at `BEGIN` time)
...
=head2 * `state` (happens at run time, but only once)
...
=head2 * Block/exceptions phasers
...
=head1 More operators thingies !

=head2 Everybody loves operators ! Let's get more of them
...
=head2 Create your own operators !
...
=head2 And to end the operator list:

=head2 * Sequence operator
...
=head2 * Generic ordering

    3 before 4; # True
    'b' after 'a'; # True

=head2 * Short-circuit default operator

    # Like `or` and `||`, but instead returns the first *defined* value :
    say Any // Nil // 0 // 5; #=> 5

=head2 * Short-circuit exclusive or (XOR)
