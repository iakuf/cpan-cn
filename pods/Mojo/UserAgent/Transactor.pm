...
这的 C<json> 的内容生成器是指内容通过 L<Mojo::JSON> 来编码后，并设置 content type 为 C<application/json>.
...
如果是 C<form> 的内容生成器, 并且请求是 C<GET> 和 C<HEAD> 时会自动的给查询的参数加入到请求中。

  # GET request with query parameters
  my $tx = $t->tx(GET => 'http://example.com' => form => {a => 'b'});

对于其它的请求，使用使用 C<application/x-www-form-urlencoded> 的 content type. 

  # POST request with "application/x-www-form-urlencoded" content
  my $tx = $t->tx(
    POST => 'http://example.com' => form => {a => 'b', c => 'd'});

参数会根据你给的 C<charset> 选项进行编码.
...
如果哈希的引用中，有 C<content> 或者 C<file>  的值，可以使用转换成 C<multipart/form-data> 的 content type， 来用于我们的上传。
...
也可以使用 C<file> 的值来指定包含路径的文件 ， 来做为你上传用的 asset 的对象，这会调用  L<Mojo::Asset::File> or L<Mojo::Asset::Memory>.
...
在上传过程中的 C<filename> 的值会自动的生成， 但是我们有需要的话也可以手动的设置。

如下，在哈希引用中多出来的值会合并到 C<multipart/form-data> 中的 http 的 header 中.
...
这个 C<multipart/form-data> 的 content type  也是可以被强行的通过 C<Content-Type> 这个 header 来手工替换的.
