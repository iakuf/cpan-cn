...
L<Mojo::JSON> 是一个简约的并且有可能是最快的纯 Perl 实现的 L<RFC 7159|http://tools.ietf.org/html/rfc7159>.

它支持标准的 Perl 的数据结构类型, 象标量, 数组引用, 哈希引用, 并会尝试使用 C<TO_JSON> 的方法来绑定这个引用.
...
一些常量名字会被 L<Mojo::JSON> 常量或者本地类似的值转换. 
...
有二个 Unicode 的空白字符 C<u2028> 和 C<u2029> 总是会转义, 这样会让 JSONP 更加容易, 还有会有字符 C</> , 以防 xss 攻击.
