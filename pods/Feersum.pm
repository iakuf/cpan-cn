...
全部的 HTTP 请求的解析和 IO 编码处理都是使用的 C 和 XS 代码. HTTP 的解析是用 picohttpparser, 这是 L<HTTP::Parser::XS> 的核心代码. 网络 I/O 是通过 libev 的 lib 来处理, 由 C<EV::MakeMaker> 来控制, 所以可以扩展 libev 和使用 C<EV>. 

在这服务器的事件循环中, 我们可以编写一些事件相关的应用内容, 使用 C<EV> 或者 L<AnyEvent> 都是兼容的.
...
Feersum 完全支持 PSGI 1.03  的 spec 包括 C<psgi.streaming>.

可以看看 L<Plack::Handler::Feersum> 的文档, 这个中提供了在 L<plackup> 和 L<Plack::Runner> 中使用 Feersum 的内容.


调用 C<< psgi_request_handler($app) >> 来注册 C<$app> 到 PSGI handler.
...
注意 SCRIPT_NAME 变量总是空的. PATH_INFO 将包含请求的 URI 的路径部分.

对于 body 的请求, 象 POST, C<psgi.input> 的变量中会包含请求的内容的句柄. Feersum 是通过 psgi.input 设置为 C<undef> 的方式来告诉你没有 body 来防止你错误的操作.
...
这个 C<psgi.streaming> 的接口也完全支持, 包含在 PSGI 1.03 中定义的 writer-object C<poll_cb> 这个回调的特性. 只要可写 Feersum 会调用 poll_cb 回调最后全部的数据会被 flushed.

在这个地方这些数据会被缓存起来, 直到回调返回才会 flushed 到 socket.
...
注意 C<< $w->close() >> 调用时,会给 writer 的最后一个引用 drop 掉.
