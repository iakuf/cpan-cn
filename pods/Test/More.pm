...
如果你的测试功能是正常的, 你通常在出错的时候, 能得到什么地方出错. 所以当失败的时候, 打印错误的信息是个好主意. 但有时并不能正常的工作. 所以在这, 我们有办法让你写出自己的诊断信息, 还比直接打开标准错误安全.
...
在保证不干扰测试的情况下打印诊断信息.  象 C<print> 一样, 会给 @diagnostic_message 的信息简单的连接起来输出.

这个的返回是 false, 以保留失败的返回.
...
也许你还记得 C<ok() 或者 diag()> 是会记住 C<open() 或者 die()> 的返回一样..

B<注意> 诊断输出的格式是会不断变化, 但这可以保证你去掉它也不会干扰测试.

=item B<note>

  note(@diagnostic_message);

这个和上面的 diag() 很象, 除了一个地方, 就是一长串的测试中消息是不会见到. 它只在详细的 TAP stream 中可见.

适合用于调试的注释放进去, 但并不表示有问题.
