...
因为记时器和其它的事件 watchers 独立于应用，错误不会自动的记录到日志，如果你想修改，可以要象下面一样订阅 L<Mojo::Reactor/"error"> 的回调。
...
大多的客户端浏览器都能接收流的响应，在 L<Mojo::UserAgent> 中也可以非常容易的以流的方法一段一段处理。
...
当你使用 L<Mojo::UserAgent> 下载大文件时，你不需要担心内存的使用，因为它会自动的给每 C<250KB> 的大小来存成临时文件. 你可以直接使用 asset 的对象来存成文件.
