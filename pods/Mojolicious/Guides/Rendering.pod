...
如果提供一些额外的象 route 之类的信息, 这时模板路径可以被自动检测到. 模板名遵循  C<name.format.handler> 规则, 其中 C<name> 是定义的 C<controller/action> 动作名或者是路由选择的名字, C<format> 默认是 C<html>, 其中 C<handler> 是指由什么模板技术来处理, 默认是 C<ep>.

  {controller => 'users', action => 'list'} -> 'users/list.html.ep'
  {name => 'foo', format => 'txt'}          -> 'foo.txt.ep'
  {name => 'foo', handler => 'epl'}         -> 'foo.html.epl'

全部的模板文件放我们应用的 C<templates> 的目录中, 我们可以通过 L<Mojolicious::Renderer/"paths"> 来自定义, 或者是放在应用 L<Mojolicious::Renderer/"classes"> 的 C<DATA> 部分. 
...
这个映射可以由 L<Mojolicious/"types"> 中来扩展. 
  ...
  sub startup {
    ...
    # Add new MIME type
    $c->types->type(txt => 'text/plain; charset=utf-8');
  }
...
到现在你可能见过了了内置的 404 （未找到）和 500（服务器错误）的网页, 当出错时会自动被渲染输出. 特别是在开发过程中, 他们是一个很有帮助的. 我们可以通过  L<Mojolicious::Controller/"render_exception">  和 L<Mojolicious::Controller/"render_not_found"> 的方法来定制自己的网页. 
  ...
  get '/divide/:dividend/by/:divisor' => sub {
    ...
    # 200
    $c->render_text($dividend / $divisor);
  };
