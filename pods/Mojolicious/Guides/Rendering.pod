...
你可以使用  L<Mojolicious::Controller/"validation"> 来验证你的 GET/POST 所提交给你的应用的参数. 这个对象所检查的甩的未知的字段都会被忽略, 所以你需要注表那些是可选 (optional) 和必须 (required). 然后才能对这些值进行检查.

Every check is performed right away, so you can use the results immediately to build more advanced validation logic with methods like L<Mojolicious::Validator::Validation/"is_valid">.
...
上面的 form 元素是由 L<Mojolicious::Plugin::TagHelpers> 这个 helper 生成.
Form elements generated with tag helpers from L<Mojolicious::Plugin::TagHelpers> will automatically remember their previous values and add the class C<field-with-error> for fields that failed validation to make styling with CSS easier.
