...
Sessions 在 L<Mojolicious> 中非常简单, 直接可以通过 L<Mojolicious::Controller/"session"> 方法来使用, 不过建议你使用更加安全的通过 L<Mojolicious/"secrets"> 来设置个密码短语.
...
这个 Controller 是用于实现详细的功能的类.

混合起来的 route 很方便, 但要最大限度地提高可维护性是非常有意义的, 所以我们需要分离操作代码和它的路由信息, 就可以达到这个目标.
