...
返回一个整数来描述 libev 是使用的什么后端(EV::BACKEND_SELECT or EV::BACKEND_EPOLL).

=item $active = EV::run [$flags]

=item $active = $loop->run ([$flags])

开始检查事件和调用的回调，它返回一个回调会调用  EV::unloop 或 flasg 非零 （在这种情况下，返回值是 true） 时或没有活动的 watcher 的引用在 loop  中(keepalive 是真值)，在这种情况下，返回值将为 false。在这可以解释为"如果为真，就还有更多的工作需要做，不能停止".

这个 $flags 的参数可以是下面的这些:

   0               如上
   EV::RUN_ONCE    阻塞最多一次(等待，但不循环)
   EV::RUN_NOWAIT  不阻塞（读取/处理事件，但不等待） 
...
喂一个文件描述符到 EV 事件. EV 会当指定的 C<$revents> 在 C<$fd> 这个文件描述符有准备就绪的通知时调用(绑定到 C<EV::READ> 和 C<EV::WRITE>) 

=item EV::feed_signal_event $signal

喂一个信号到默认的 loop. 如果指定的信号 C<$signal> 发生时 EV 会调用这个.
