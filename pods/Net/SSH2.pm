...
接收一个句柄在这个连接的 SSH 2 protocol.这个句柄可以是:
...
使用"键盘交互"进行认证。 要么需要密码或一个回调的代码引用来调用。这个回调需要 C<callback-E<gt>(self, username, name, instruction, prompt...)>.
根据提示产生的结果是一个哈希，提示的文字和输入产生键值对来分别进行呼应。会返回一个数组的响应。

如果只提供了用户名，默认的回调会调用标准的交互来响应，这时需要 L<Term::ReadKey> 的支持.
...
创建并返回一个新的通道(channel)对象。默认类型为 "session".  See L<Net::SSH2::Channel>.
...
返回 public key 接口的对象 (see L<Net::SSH2::PublicKey>).

=head2 poll ( timeout, arrayref of hashes )

传一个以毫秒为单位的超时和下面为 key 的哈希的数组引用:

=over 4

=item handle

也许是  L<Net::SSH2::Channel>  或者 L<Net::SSH2::Listener> 的对象，整数文件描述符或 Perl 文件句柄。
