...
Net::SNMP 模块实现了面向对象的简单网络管理协议接口. Perl 应用可以(通过 SNMP 协议)
使用这个模块对远程主机进行信息检索或更新. 这个模块支持 SNMP 版本1, 版本2c, 和
版本3. Net::SNMP 模块假定用户对 SNMP 协议和相关网络管理概念有了一个基本的了解.
...
Net::SNMP 模块把原本复杂的 SNMP 协议通过在原协议的基础上提供了一个抽象化的高级的编程接口.
每个 Net::SNMP 对象都在 Perl 对象与远程 SNMP 客户端或管理端之间提供了一对一的映射. 一旦
一个对象被创建, 它可以用来执行被 SNMP 定义的基本协议交换操作.

一个 Net::SNMP 对象被创建, 那么它会拥有"blocking"或者"non-blocking"属性. 默认情况下, 这些发 SNMP 信息的方法是不会返回的, 直到这个协议交换完成或者已到超时时间才会返回.
这个行为给了这个对象一个"blocking(锁)"的属性, 因为在方法的返回到来之前, 代码的执行流程会处在停滞状态.

在创建对象的时候可以设置可选参数B<-nonblocking>为true到对象的构造函数, 这样可以得到一个无阻塞的对象. 当一个无阻塞对象被调用时, 队列中的 SNMP 消息将立即返回, 并且这个代码流程将继续执行.
在一个通过调用C<snmp_dispatcher()>方法的循环事件以前, 队列中的SNMP 消息不会被发送. 在 SNMP 消息发送时, 所有关于这个消息的响应都会调用由用户(原本放入队列的消息)定义的子程序.
...
在这个方法完成以前,这个对象关联的方法默认行为是阻止代码流程进行. 对于那些在初始化时要求响应的 SNMP 协议交换方法, 一个包含请求结果的哈希引用将被返回. 当所有方法中有一个发生了失败, 一个未定义的值将被返回.
"C<error()>"这个方法可以用来确定失败的原因.
