...
AnyEvent::Handle - AnyEvent 的非阻塞  I/O 处理 
...
这是一个帮助，告诉你怎么在流上写基于事件 I/O 句柄的操作(sockets，管道和其它的流处理).

在  L<AnyEvent::Intro> 包含着一些 AnyEvent::Handle 的示例.

在下面的文档中指的 "bytes" 是意味着字符，象 sysread 和 syswrite 能在所有的 I/O 的地方操作他们。所有和字符有关的模块也一样.

最少你需要指定  C<fh>  或 C<connect>, 和 C<on_error>  的回调.
...
构造函数支持这些参数 (全部是 C<< key => value >> 对).

=over 4

=item fh => $filehandle     [C<fh> or C<connect> MANDATORY]

这个句柄会使用 L<AnyEvent::Handle> 对象来操作。

注意： 这个句柄会被构造函数设置成 non-blocking mode (使用 C<AnyEvent::Util::fh_nonblocking>) 。

=item connect => [$host, $service]      [C<fh> or C<connect> MANDATORY]

尝试连接到指定的主机($host)和服务($service 端口), 使用的是 C<AnyEvent::Socket::tcp_connect>. 这个 C<$host> 会变成默认的 C<peername>。

您必须指定使用此 C<fh> 参数.

它可能会推请求到读和写的队列上，并修改流的属性，使用 AnyEvent::Handle 来操作连接.

当设置了  C<on_prepare>, C<on_connect_error> 和 C<on_connect> 的回调，会在适当的时候调用这些回调。

=over 4

=item on_prepare => $cb->($handle)

这个回调并不常用，它是在创建连接之前，创建句柄之后被调用，所以这时已经可以访问 C<< $handle->{fh} >> 了。它可以用于设置句柄所需要的连接所使用的参数，因为还没有建立连接，所以可以更改设置.

这个回调要返回连接超时的值以秒为单位(C<0> 或 C<undef> 或 空的列表，以表明默认要使用的超时时间)

=item on_connect => $cb->($handle, $host, $port, $retry->())

当连接已经成功建立时，调用这个回调函数。

这行中的 $host, $port 是做为套接字的参数，加上重试的回调。在这个连接的时候会调用读和写的队列，其中有一些如 EOF 的状态,TLS 的状态和类似的句柄都会被 reset.

在这个地方是不能使用读写队列来处理句柄的连接的。

如果因为一些原因，这个句柄不能用的时候，就会调用 $retry 来连接下一个目标(例如，多个解析的地址记录)。在连接调用返回后就会调用 $retry 的调用。一开始建连的时候，如果失败会连接下一个主机地址.

在大多的时候，你并不需要使用 $retry 的参数.

=item on_connect_error => $cb->($handle, $message)

当无法建议连接时，这个回调会被调用， C<$!> 会包含相关的错误代码， C<$message> 会包含一些消息(通常是和 C<"$!"> 一样);

        
如果不指定此回调，后面的 C<on_error> 将调用一个致命错误

=back

=item on_error => $cb->($handle, $fatal, $message)

这是错误时的回调，当调用这个时,说明有一些错误出现，例如象不能解析主机名，失败的连接，读错误.

有些错误是致命的( C<$fatcl> 会被设置成真).当出现 fatal 的错误,句柄的对象会被 destroyed (通过调用 C<< -> destroy >>), 后才调用错误回调，所以你可以随意检查句柄对象。常用的致命错误的例子是当一个活动的读 watcher （C<EPIPE> 进程没处理 Socket 错误继续向 TCP 写入数据）时发生 EOF 条件或 I/O 错误。

AnyEvent::Handle 会试图找到对应合适的错误代码，但有些时候象 (TLS 错误),并不一定能很好的工作.

如果您报告错误给用户，建议在人类可读的错误信息 (所以并不建议你直接输出 C<"$!"> );

如果你真想输出这些错误 ，你最好看看和 C<Errno> 文档来比较一下 C<$!> ，然后提供信息.

只要是非致命错误(  C<Non-fatal> ),你可以简单的重试和忽略，常用的非致命错误的例子是超时  C<ETIMEDOUT> 和 C<EBADMSG>。

在进入回调时， C<$!> 会包含着操作系统的错误代码(C<ENOSPC>, C<EPIPE>, C<ETIMEDOUT>, C<EBADMSG> or C<EPROTO>).

虽然不是强制性的, 但我还是建议设置这个回调，这样不会出现很多错误报告，不然会调用 C<croak>.

=item on_read => $cb->($handle)

这是设置默认的读回调, 就是当有数据到达(可读时)，并且没有任何读取请求在队列中时调用,这个不象 push_read 需要条件成立时才读，这个只要有一个字节都能读取.

你可以访问和删除 read buffer.使用 C<< ->rbuf >> 的方法来访问  C<< $handle->{rbuf} >> 就行了。注意你不能增加和修改 read buffer.但是可以删除.

你可以调用 C<< ->push_read (...) >>  和其它相当的功能来修改 read queue.

当 EOF 条件成立的时候, AnyEvent::Handle 会首先试着给队列中的数据来使用  C<on_read> 这个回调处理，处理完后调用  C<on_eof> 的回调。如果这个过程出现了问题全设置 C<$!> 成  C<EPIPE>。

注意, 不同于读取的队列中的请求，使用 on_read 是可能读不到数据，比如读到了一个 EOF 但时还有未完成的读取请求，但这时会标记成错误，所以当 on_read 调用时，就会调用 on_eof.

=item on_eof => $cb->($handle)

当检查到文件结束的时候调用本设置的回调函数。当一个 socket 连接时，对方干净利落地关掉了连接并且队列中没有未完成的读请求。如果这时还有读请求，这个 EOF 被认为是意外的连接关闭，会标记为错误。

对于套接字，这也就意味着，对方已经停止发送数据，你仍然可以尝试写数据时， 因为可以返回 EOF 回调和还是能继续写入数据，因为只有读的部分已经被关闭。

如果发现了 EOF 但并没有设置 C<on_eof> 的回调函数，这时 fatal 的错误中  C<$!> 会设置成 0.

=item on_drain => $cb->($handle)

当写的 buffer 变成空的时候，会调用这个设置的回调。(设置可以在句柄设置的时候创建这个).

我们追加东西到写缓冲区中，只要使用 C<< ->push_write >>  的方法就行了。

这个回调非常的有用，当你不想给的有的数据一次性的写入到写队列时。例如：当你想给一些文件的内容写到 socket 中。但你并不想一次给整个文件都读到内存中来然后推到队列中。所以我们常用的方法是当写队列变成空的时候我们读更多的内容追加写到写队列。
...
上面的参数只要不是为零，对于不活动的连接就会激活超时断开: 当这些句柄正常的读和写时超过多少秒时(或调用 C<timeout_reset>)，这个 C<on_timeout> 的回调就会被调用(如果这个回调不存在，会引起 C<ETIMEDOUT> 的错误).

在 AnyEvent 中有三种不同的超时，这三种不同的超时相互独立，同时作用于读取和写入的超时（触发在没有读 I<OR> 写时),只和读有关的超时(触发在没有读)和只和写有关的超时(触发没有写时):  C<timeout>, C<rtimeout> and C<wtimeout>,与相应的回调是  C<on_timeout>, C<on_rtimeout> 和 C<on_wtimeout> 和重置功能  C<timeout_reset>, C<rtimeout_reset>, 和 C<wtimeout_reset>。

注意这个超时，如果你一段时间没有读取和写入的请求时，只要打开了它就会工作，所以如果你打算保持这种空闲的连接，你需要暂时禁用超时或忽略  C<on_timeout> 这个超时的回调，象  AnyEvent::Handle 中会直接重设超时.                                                                                                                                                                                
...
闲置多久后超时然后调用回调。如果这个回调 return 时这个超时会被重置。只要有活动发生，这个就不会发生.

=item rbuf_max => <bytes>

如果定义了这个参数，当读的缓冲区超过这个大小时，会引发一个致命错误(会给  C<$!> 设置成 C<ENOSPC>)。这是个很严格的检查，但这也很有用，可以避免某些形式的拒绝服务攻击。

例如，有时不受信任的源服务器连接上来有一些不法的行为，这些攻击者发送无限量的数量包，然后你的回调因为一直在接收并不回调也没有回调的截止字符。这样你的服务一下就能挂掉.

=item wbuf_max => <bytes>

如果定义了这个参数，当写的缓冲区超过这个大小时，会引发一个致命错误(会给  C<$!> 设置成 C<ENOSPC>)。这是个很严格的检查，但这也很有用，可以避免某些形式的拒绝服务攻击。

虽然此参数的单位是字节，但这是指尚未接受由内核提供的原始数量的字节。这是有用的，比如在 TLS 中，因为 TLS 通常写入的数据较大(当然也有可能更小，可以压缩).

这种限制，比如在聊天的聊天消息发送到客户端的服务器。如果客户端不及时阅读这些服务器发送缓冲区中会无限增长。

=item autocork => <boolean>

默认是禁用， C<push_write> 的实现是只要可能会尝试立即给数据写入句柄中。这样就没有必要给注册  write watcher 并等待下一次事件循环迭代，但是如果你要写入很多小块数据时这样比较低效。（这种缺点通常避免由您的内核 nagle 算法，请参阅 C <no_delay>）。

启用时，写操作将总是排入队列直到下一次的事件循环迭代。当你每次迭代 （或当写入缓冲区满了） 仅写一次，这时如果写入很多小数据合并时非常有效，不然效率较低。它还增加了写入滞后时间。

=item no_delay => <boolean>

当你写在 sockets 上写少量数据时，你的操作系统的内核通常会在发送这些数据之前，等更加多的数据然后一起发送。这调用和设置的是 Nagle 的算法，这个可能对你可能有用。

如果你想有更加低延时数据发送，你可以设置这个选项为 true .

这个默认是使用操作系统的缺省的行为(大多是加打开这选项).  这个选项提供给你一个明确的可以设置它的地方。

=item keepalive => <boolean>

启用(默认是禁用)  stream socket 中的 SO_KEEPALIVE 的选项。通常情况下，TCP 连接一旦建立并不会超时。所以，可以一直保持活的状态。TCP 的连接保持是一种非常好的提高传送性能的方式。使用长保持 TCP 连接可以直到另一边的连接不可用时。默认的超时是依赖于操作系统的，通常能在开始生效后保持大约 2 小时。当另一端的连接没有反应时，断开这个 TCP 连接大约是 10 -15 分钟后.

大多的时候指定这个选项，并不会产生什么不好的影响。除非不支持这个选项。打开这个选项通常是个非常好的主意。
...
初始的读取块的大小, 这个数是每次这个循环迭代会尝试读取的字节. 每一个句柄最少需要使用这个设置的内存, 所以处理许多连接的时候, 注意内存的使用). 另外可以
看 C<max_read_size>. 本参数的默认值为 C<2048>.
...
这个方法直接返回 L<AnyEvent::Handle> 对象建议的文件句柄.
...
替换当前的 C<on_error> 的回调( 看 C<on_error> 上面的说明);
...
在队列中写入一些标题的数据. 你也可以推一些你想推的数据 (注意 C<wbuf_max> 别超这个的限制就行), 你可以使用 
...
=head2 READ QUEUE 读队列


AnyEvent::Handle 管理着二个句柄，一个写一个用于读。

读取的队列是比写入队列更复杂。它可以使用两种方式，"简单"的方法是直接使用 C<on_read> 另外一种是"复杂"的方式，使用队列。

在最简单的情况下，如果你只使用了 C<on_read> 的回调，这会在每当数据到达时就会被调用。这时你可以直接使用 C<< $handle->rbuf >> 从读缓冲区中移除一些数据.或者你想保留来积累更多的数据(例如到目前只收到了部分消息)，或者更换成读取队列来处理 C<push_read>.

在更加复杂的例子中，你想在队列中使用多个回调，比如， 在 AnyEvent::Handle 在每次有新的数据到达时调用队列第一个回调并在完成工作后删除这些数据(看下面的  C<push_read>).

使用这种方式，象下面这样，多了三行读，下面的是读 chunk 的数据，AnyEvent::Handle 会按顺序来执行。

示例 1： EPP 协议分析器。EPP 发送 4 字节长度信息，其次是指定给一个 XML 数据报的字节数。

   # 在默认状态下，期望某些头字节
   $handle->on_read (sub {
      # 当收到一些数据,对头的长度进行读取 (4 octets)
...
返回的读取的缓冲区（也可作为一个可修改的的左值）.你也可以直接通过 C<< ->{rbuf} >> 来访问，这样会更加快些。

这只能做读缓冲的操作，也就是删除数据。其它的修改和追加是不允许的，这导致难以查的错误。

注意： 读缓冲只能在 C<on_read> 的回调中使用和修改。要么使用 C<push_read> 和 C<unshift_read> 的单个回调。 C<push_read> 和 C<unshift_read> 的方法会自己管理读缓冲区。
...
C<push_read> 是在最队列的结尾加入一个新的回调，C<unshift_read> 是在所有队列的前面加上这个回调。

回调的时间是当每次有一些额外的读数据到达时回调。

上面的方法需要检查是否有足够的数据已经是在读缓冲区。

如果没有足够的数据也是可用的，它必须返回空列表或假的值，在这种情况下，它会被反复调用，直到足够的可用数据（或检测到错误条件）。

如果有够的数据，你的回调需要删除你所感兴趣的这些数据(which can be none at all)并返回 true 。这时才会给给这个从队列移除.

这些方法可能会调用一些其它回调函数（所以它返回后句柄可能会销毁），
...
使用正则表达式的对象 C<$accept> 来进行正则表达式匹配,并返回所有的正则之前的东西和匹配的内容.

例如: 读一个以  '\n' 做为结束符的单选.

   $handle->push_read (regex => qr<\n>, sub { ... });

如果 C<$reject> 这个地方定义了时(不为 undef)，那么当出现这种确认的数据时就会被拒绝：它是指当 $accept 的正则不匹配时，而这个本身匹配的数据时全生成一个 C<EBADMSG> 的错. 

这是非常有用的，可以快速地拒绝错误数据（以避免等待超时或接收缓冲区溢出）。

例如：我们需要一个十进制数,之后是空白,只要不对,拒绝其他任何东西.

   $handle->push_read (regex => qr<^[0-9]+\s>, qr<[^0-9]>, sub { ... });

如果 C<$skip> 个地方定义了时(不为 undef)，那么当 C<$accept> 找到了相关的内容和 C<$reject> 并不匹配时，它会检查接收到的 buffer.这是非常有用的，可以跳过你知道不匹配大量的数据.


这是纯粹的优化，通常是值得的，大量要跳过的数据的时候, 使  C<$accept>  or C<$reject> 的正则表达式就不需要从一开始就匹配找起.

例如: 我们需要抓 HTTP 的 header, 结束符是 C<\015\012\015\012>. 由于我们预计 header 是非常大的。 

我们使用 skip 的正则表达式跳过起始部分.  在这 skip 的正则表达式有点刁钻，因为 \015 和 \012 都是结束符.所以这些都需要使用正则。
...
=item 为什么我的 C<on_eof> 回调从来没有被调用过?

可能是因为你的 C<on_error> 回调被其它的方法替代： 当你的读队列中有没有完成的请求，然后
Probably because your C<on_error> callback is being called instead: When you have outstanding requests in your read queue, then an EOF is considered an error as you clearly expected some data.
...
大多数协议包括二个阶段，某些的设置阶段（例如身份验证），然后通过是请求处理阶段，服务器在一个循环中等待请求并处理他们。

这有二种不同的情况： 第一种传统的比较好的处理请求交互的方式，是服务器直接从对端到得到了 QUIT 之类的命令，然后在这个时候立即关掉连接（繁忙的 TCP 服务器上这个非常可取）。另一个是客户端退出这个连接出现一些错误，这时你可以检测并需要对关闭时发现一些意外检查。
...
To handle this case, always make sure you have a non-empty read queue, by pushing the "read request start" handler on it:
   ...
   # 现在推第一个 @start_request
   $hdl->push_read (@start_request);

对于有些不确认的数据在 C<push_read> 中，这个句柄在连接意外断开时总是期望得到一些数据要么得到 C<EPIPE> 的错误。

这第二种情况，有些协议可以让客户端在任何时候都能断开连接。 象 TCP 协议, 这样的话，服务器上运行 sockets 之类协议一般情况下更加容易，但这也意味当一个正常的关闭时你不能区分协议失败还是客户端崩溃这二种情况。然而，这些种类的协议很常见 (和有时甚至是解决问题的最佳办法)。

如果你忽略 C<EPIPE>  的错误时，还在任何时候都有可靠的读取是可能存在的，但这并没有什么用，因为当客户端在一个请求期间断开连接时，将仍然有错误出现。

这样最好的解决方案是在初始请求读取推给 C<on_read> 回调.这样可以在当服务器有非期望的数据时避免这样的错误.在这时会等着下一个请求进来，EOF 不会引发错误，只是你需要在 C<on_eof> 的回调中来读取结果。这样会有点慢，但是简单。 
...
=item 我该如何读取数据，直到对方关闭了连接, 

如果你只是想读一些数据到 Perl 中。非常容易实现， 只需要设置 C<on_read>  的回调不做任何事情和清空  C<on_eof> 。然后在设置一些在  C<on_error> 的回调中处理 C<$_[0]{rbuf}> 的数据就好了。
...
注意这个例子中从对象的句柄中删除 C<rbuf> 的成员，这并不是一个标准的 API 。如果标准的删除这个对象，需要析构这个对象。

上面之所以在 TCP 的连接中使用 C<on_error> 来读取数据是因为有可能因为延迟和数据包丢失，连接突然关闭，但其实已经收到的所有数据。

它通常是更好的方法是，一定要使用确认机制，在数据传输时确保对方真的断开了，你有得到了完整的数据。这也是为什么有这么多的网络协议有一个明确的 QUIT 命令的原因。
