...
有些错误是致命的(会通过 C<$fatcl> 会被设置成真).当出现 fatal 的错误时，调用错误回调后 handle 的对象会被 destroyed (通过调用 C<< -> destroy >>).
Some errors are fatal (which is indicated by C<$fatal> being true). On fatal errors the handle object will be destroyed (by a call to C<< -> destroy >>) after invoking the error callback (which means you are free to examine the handle object). Examples of fatal errors are an EOF condition
...
If non-zero, then these enables an "inactivity" timeout: whenever this many seconds pass without a successful read or write on the underlying file handle (or a call to C<timeout_reset>), the C<on_timeout> callback
