...
使用 fork 来异步执行给定的代码块.这个块的所有返回都会给到原调用的进程, 通过 Storable 的 serialising 和 deserialising .

如果有任何错误, 那么 C<$cb> 的调用时会没有任何参数. 在这种情况下，要么 C<$@> 包含着异常或者 C<$!> 中包含着错误数. 其它正常的时候 C<$@> 会是 C<undef>.

代码块必须不要使用事件轮询功能或直接使用基于事件的编程，不然可能会导致在父进程中的回调运行。

Win32 spoilers: Due to the endlessly sucky and broken native windows perls (there is no way to cleanly exit a child process on that platform that doesn't also kill the parent), you have to make sure that your main program doesn't exit as long as any C<fork_calls> are still in progress, otherwise the program won't exit. Also, on most windows platforms some memory will leak for every invocation. We are open for improvements that don't require XS hackery.
...
这个功能用于指定一个特定的代码块的对象，当这个块被调用时，执行这个代码块中的内容.

这通常用于清理一些资源，当你在这个代码的过程块中的时候，无论你从那退出都会执行.

如果 L<Guard> 的模块是可用的，会用这个模块来实现这个功能，不然使用纯Perl实现。

当代码可以在不正常的时候抛出异常,但这个模块没定义异常报告(目前, Guard 模块和 AnyEvent 纯 Perl 实现尝试报告错误并继续).

你可以直接调用下面的方法返回这个对象:

=item $guard->cancel

这样的调用会让上面自己定义的代码块中的内容不被调用: 它是 "取消" guard 的意思.
