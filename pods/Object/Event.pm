...
这有一些语法糖, 可以让你调用 L<Object::Event> 的派生类对象实例上的方法. 下面 L<EVENT METHODS> 部分有详细的内容.
...
如果你的这个类没有 C<new> 的构造函数, 你需要调用这个方法来初始化事件系统.
...
这个方法注册回调 C<$cb1> 名为 C<$eventname1>. 你也可以注册多个 eventname => callback 的对.

这个返回 C<$guard> 的值, 这个 guard 代表你所有的设置的回调集. 你可以使用这个 C<$guard> 来注销回调或调用  C<unreg_cb> 来通过 ID 来删除回调.
如果  C<reg_cb> 调用的时候, 是一个空的上下文时没有 guard 被返回时, 你会没有机会注销注册过的回调.

注册的回调的第一个参数总是 master object C<$obj>.

回调的返回值会被忽略. 如果你需要给信息传递处理程序, 你必须建立自己的"协议"，要实现这个。我建议传递数组引用的处理程序：
