...
Term::ReadKey - 一个 Perl 模块进行简单的终端控制
...
Term::ReadKey 致力于提供简单的控制终端驱动程序模式 (cbreak, raw, cooked, etc.,) 如果体系结构允许,支持无阻塞读取和一些常用的终端工作上方便的功能。主要目标之一是可能实现可移植性功能，所以 "use Term::ReadKey" 可以在任何架构上比较好的工作.
...
模式是整数参数，目前可以是下列值之一:
    ...
    0    恢复原来的设置。 
    1    改变到 cooked mode.
    2	 改变到 cooked mode 并关掉回显. 
          (主要用于输入密码)
    3    Change to cbreak mode.
    4    Change to raw mode.
    5    改变到 ultra-raw mode. 
          (LF 到 CR/LF 翻译关闭) 
...
如果没有提供其它的句柄，这些功能默认是应用到 STDIN 的句柄。模式 0 和 5 值得一提的有一些特殊的特性: 不仅模式0恢复原来的设置，但它会导致下一个ReadMode调用保存一组新的默认设置.模式 5 和模式 4 类似, 只是 5 没有 CR/LF 转换的功能.如果可能会禁用奇偶校验.(终端不使用时生效，但在 Windows 下他们是一样的).

如果您正在执行另一个程序，可能会改变终端模式，你可以
...
这会记录任何 程序可能产生的改变，在复位模式之前.

=item ReadKey MODE [, Filehandle]

得到一个整形的参数，你当前可以使用下面的值:

    0    Perform a normal read using getc
    -1   Perform a non-blocked read
    >0	 Perform a timed read

如果句柄没有提供，它默认作用于 STDIN。
