...
只要查询的语句没有问题的话，任何方法的调用，都会返回 DBIx::Class 中查询结果的返回集在 resultset 中。
...
如果 L</Unique constraints> 有定义，我们可以使用 C<find> 来对行基本这个查找，我们也可以在 C<find> 的时候使用 unique 的值来替换默认的 primary key 的查找方式。我们只需要设置象下面的 key 指定为 unique 的值就好了。
...
从结果集中取得多行的集要怎么办啦，我们可以简单的给这些行对象全都转换成数组。如果你的结果很大的话，你可以使用 while 来使用少一些内存。
...
下面这个就是使用 while ，这时在同一时间只从结果集中取一行数组出来，所以这样比上面的方法会少使用很多内存.
...
有时我们只需要使用查找到的第一行的数据，我们可以使用  C<first>  来取得首行的结果。这常常用在排序过的结果后，然后来取这个.
