...
当 $var 是一个对象的实例时,你可以可以调用它的方法通过"."这个操作符.
...
在派生模板中你可以扩展自己的模板, 例如我们使用 F<myapp/foo.tx> 通过块修饰方法 (block modifiers).

    : # 给 "myapp/base.tx" 级联进来
    : cascade myapp::base
    : # 使用默认的标题，所以 title 块不能写出来，只修改 body 块. 
    : around body -> {
        My template body!
    : }

然后, 你还可以修改你的模板 F<myapp/bar.tx> ,这次我们显示的使用 title 并使用默认值:
...
你可以替换掉基础模板, 如下新的例子中只要在你的文件中 F<myapp/hello.tx> 来做基础模块: 
