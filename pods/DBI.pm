...
默认值为 false, 正常的情况下, 当最后一个引用计数删除时, 这个句柄会被 destroyed 掉.

如果设置为 true, 句柄在不活动的时候会才会被销毁, 所以数据库引擎相关的调用句柄 DESTROY 的操作都会跳过并失效. 
从这名字中就能看出, 这个就是用于处理不活动的句柄销毁用的方法.

对于一个数据库句柄, 这个属性不会禁用显式调用 disconnect 方法, 只有当句柄仍被标记为活跃, 但又发生了 DESTROY 的调用时才会禁用.

这个属性专为 Unix 应用的 'Fork' 子程序所设计的. 有一些数据库驱动, 当子进程退出时想销毁继承的句柄时, 因为和父进程共用所以引起父进程停止工作.

在父进程或者子进程上所有共享的句柄上设置 C<InactiveDestroy> 为真, 注意不能二个同时设置. 要么在只在父进程上设置  L</AutoInactiveDestroy> 就行了. 

为了帮助跟踪使用了 fork 的应用程序, 进程 id 可以在 trace log 中显示出来, 当你使用 DBI 或者句柄调用 trace() 方法时. 在这, 进程的 id 会显示在每次方法调用的时候. 需要设置 DBI trace 级别, 不是句柄的级别. 这个需要设置大于 9.

=head3 C<AutoInactiveDestroy>

类型: 布尔, 继承

如上所述, 因为  L</InactiveDestroy> 这个属性需要显示的设置在子进程调用 fork() 之后. 这是一个大问题, 当不在我们控制的情况下, 我们使用了第三方模块,所可能有影响, 所以我们可以使用 C<AutoInactiveDestroy> 来解决这个问题.

如果设置为 true. 当调用 DESTROY 的方法时会自动的检查当前句柄上的进程的 id . 如果是不同的进程 id 会自动的设置 I<InactiveDestroy> 的属性.

这是它的设计处理的例子：

    my $dbh = DBI->connect(...);
    some_code_that_forks(); # 也许在您不知情的情况下被 fork 了 
    # 子进程死掉后, 会调用继承的 DESTROY 方法
    $dbh->do(...); # 出错, 因为父进程的 $dbh 现在不是可用的.

这个 C<AutoInactiveDestroy> 的属性在 DBI 的 1.614 才被加入.
...
这个工具方法绑定 L</prepare>, L</execute> 和 L</fetchall_arrayref> 到单次调用, 返回
This utility method combines L</prepare>, L</execute> and
L</fetchall_arrayref> into a single call. It returns a reference to an array containing a reference to an array (or hash, see below) for each row of data fetched.
...
尝试判断数据库服务器是否仍在运行并且连接是否仍然工作的一个有效的方式. 各自的数据库驱动程序需要为自己的数据库引擎实现最合适的功能.

当前 I<默认> 的实现是不做任何事件只返回真. 实际上, 它返回的是 "C<0 但这为真>". 在这零为真. 这样你可以告诉用户如果返回值是真或者是默认. 驱动程序有必要重官吏一个正确判断数据库是否的方法.

好多应用程序不得不直接使用这个方法, 请参考 Apache::DBI 模块的例子.
